...
def valid_taxa(cur, csv_template, yml_dict):
    ...
    for element in taxa_dict:
        response["message"].append(
            f"  === Checking Against Taxa {element['taxonname']} ==="
        )
        ...
        if len(result["match"]) == 0:
            response["message"].append(
                f"  ✗ No approximate matches found for {result['name']}. Have they been added to Neotoma?"
            )
            ...
            response["message"].append(
                f"  ? No exact match found for {result['name']}, several potential matches follow:"
            )
            matches.append(False)

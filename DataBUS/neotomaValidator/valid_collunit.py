...
def valid_collunit(cur, yml_dict, csv_file):
    ...
    """
    response = CUResponse()
    params = ["handle", "core", "colltypeid", "depenvtid", "collunitname", 
    ...
    try:
        cu = CollectionUnit(handle = inputs['handle'], 
                            core = inputs['core'], 
    ...
    if not rows:
        ...
        if len(close_handles) > 0:
            ...
            if any([j == cu.handle for j in goodcols]):
                response.message.append(f'?  A collection unit with this name '
                                        f'already exists nearby.')
            else:
            ...
            response.message.append(f'✔  There are no nearby sites, a new collection unit '
                                    f'will be created.')
            response.valid.append(True)
    return response
...
def valid_datauncertainty(cur, yml_dict, csv_file, validator):
    ...
    assert len(validator['taxa'].uncertainty_inputs) == len(inputs), "Taxa Uncertainties and extracted uncertainties do not match."

    taxacounter = 0
    for i, uncertainty in enumerate(inputs):
        assert (len(uncertainty['uncertainty']) == 
                validator['taxa'].uncertainty_inputs[i]), (f"Number of "                                       
        ...
        for j in range(len(uncertainty['uncertainty'])):
            ...
            try:
                DataUncertainty(dataid = taxacounter, # retrieve correct ID for insert

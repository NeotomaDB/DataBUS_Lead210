...
def insert_datauncertainty(cur, yml_dict, csv_file, uploader):
    ...
    assert len(uploader['data'].uncertaintyinputs) == len(inputs), "Taxa Uncertainties and extracted uncertainties do not match."
    taxacounter = 0
    for i, uncertainty in enumerate(inputs):
        assert (len(uncertainty['uncertainty']) == 
                len(uploader['data'].uncertaintyinputs[i]['dataid'])), (f"Number of "                                       
                    f"uncertainty values does not match number of data values")
        ...
        for j in range(len(uncertainty['uncertainty'])):
            ...
            try:
                du = DataUncertainty(dataid = int(uploader['data'].uncertaintyinputs[i]['dataid'][j]), # retrieve correct ID for insert

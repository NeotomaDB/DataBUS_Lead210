...
def insert_collunit(cur, yml_dict, csv_file, uploader):
    ...
    """
    response = CUResponse()
    params = ["collectionunitid", "handle", "core", "colltypeid", "depenvtid", "collunitname", 
              "colldate", "colldevice", "gpsaltitude", "gpserror", 
              "waterdepth", "substrateid", "slopeaspect", "slopeangle", 
              "location", "notes", "geog"]
    inputs = nh.clean_inputs(nh.pull_params(params, yml_dict, csv_file, 'ndb.collectionunits'))
    try:
    ...
    if cu.handle:
        ...
        if len(coll_info) == 1:
            ...
            try:
                found_cu = CollectionUnit(collectionunitid = int(coll_info[0]),
                                          ...
                                          geog = Geog((nh.clean_numbers(coll_info[8]), 
                                                       nh.clean_numbers(coll_info[9]))),
            ...
            response.culist.append({'original site': f"{cu}", 
                                    'updated site': f"{updated_cu}"})
            cur.execute(upsert_query) 
            try:
                response.cuid = updated_cu.upsert_to_db(cur)
                response.valid.append(True)
                response.culist.append({'collunit': cu, 
            ...
            if overwrite['handle'] == True:
                ...
                response.valid.append(False)
                response.message.append(f"✗  Overwrite is set to False. "
                                        f"Collection Unit ID {response.cuid} is not "
                                        f"currently associated to a Collection Unit in Neotoma.")
        ...
        try:
            ...
            cur.execute("ROLLBACK TO SAVEPOINT before_try") # Clear status of previous error.
            response.message.append(f"✗ Collection Unit Data is not correct."
                                    f"Error message: {e}")
            response.cuid = cu.insert_to_db(cur)
            response.culist.append(cu)
            response.valid.append(False)

...
def insert_analysisunit(cur, yml_dict, csv_file, uploader):
    ...
    """
    params = ["analysisunitname", "depth", "thickness", "faciesid", 
              "mixed", "igsn", "notes", "recdatecreated", "recdatemodified"]
    inputs = nh.clean_inputs(nh.pull_params(params, yml_dict, csv_file, 'ndb.analysisunits'))
    response = AUResponse()

    kv = {'mixed': False, 'faciesid': None, 'igsn': None, 
          'notes': None, 'recdatecreated': None, 'recdatemodified': None}
    for k in kv:
        if inputs[k] == None:
